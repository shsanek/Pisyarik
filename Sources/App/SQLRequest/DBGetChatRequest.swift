...
extension DBGetChatRequest {
    init(userId: IdentifierType) {
        ...
        self.request = "SELECT chat.is_personal as is_personal, chat.identifier as identifier, chat.name as name, message.user_id as author_id, user.name as author_name, message.chat_id as chat_id, message.body as body, message.date as date, message.type as type, message.identifier as message_id FROM chat_user, chat, message, user WHERE chat_user.user_id = \(userId) AND chat_user.chat_id = chat.identifier AND message.identifier = chat.last_message_id AND user.identifier = message.user_id;"
    }
