struct DBAddMessageRequest: IDBRequest {
    ...
    var request: String {
        ...
        UPDATE chat_user SET not_read_message_count = not_read_message_count + 1 WHERE chat_id = \(message.chat_id) AND user_id <> \(message.author_id);
        UPDATE chat_user SET not_read_message_count = 0, last_read_message_id = @last_identifier WHERE chat_id = \(message.chat_id) AND user_id = \(message.author_id);
